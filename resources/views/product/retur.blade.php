@extends('layouts.app')
@section('title','Daftar Barang retur')
@section('content')
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-9">
            <div class="card">
                <div class="card-header">Daftar Barang retur</div>

                <div class="card-body">
                    @if($product->count() > '0')
                        @if(auth()->user()->role == 'pegawai')
                        <button class="btn btn-danger delete_all" data-url="{{ url('produk_retur') }}">Hapus Semua</button>
                        @endif
                	<a href="/produk/retur/export" class="btn btn-success">Export Excel</a>
                                ...
                                @if($product->count())
    	                            @foreach($product as $item)
    	                            	<tr id="tr_{{$item->id}}">
    	                            		...
    	                            		<td>{{$item->satuan}}</td>
    	                            		<td>
    	                            			<a href="/produk/retur/{{$item->id}}/edit">Edit</a>
    	                            		</td>
    	                            	</tr>
    	                            @endforeach
                                @endif
        ...
        <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                  <div class="modal-dialog" role="document">
                    ...
                    <div class="modal-body">
                        <form method="post" action="/produk/retur/add">
                            {{csrf_field()}}
        ...
        <div class="modal fade" id="lol" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                    ...
                    <div class="modal-body">
                        <form action="{{ url('/produk/retur/excel') }}" method="post" enctype="multipart/form-data">
                            @csrf
