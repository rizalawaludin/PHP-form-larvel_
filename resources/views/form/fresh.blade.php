@extends('layouts.web')
@section('title','Pemesanan Produk Fresh')
@section('content')
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">Pemesanan Produk NutriFood Fresh</div>
                <div class="card-body">
...
<script>
    $(document).ready(function(){
        ...
        function dynamic_field(number)
        {
                ...
                html += '<td><select name="product[]" class="form-control" required><option value="">-pilih produk-</option>@foreach($product as $item)<option value="{{$item->nm_produk}}">{{$item->nm_produk}} (<small>Rp.{{$item->harga}}</small>) (<small>{{$item->satuan}}</small>)</option>@endforeach</select></td>';
                html += '<td><input type="number" name="jumlah[]" class="form-control" min="0" required /></td>';
                if(number > 1)
                {
